...
mcmc_run_post <- function(mcmc_run_all_output,
  ...
  if(isTRUE(relabel)){
    ...
    label.changing.table <- data.frame(original.label = original.label,
                                       new.label = 1:J)

    index <- sapply(1:nrow(label.changing.table),
                    function(i) which(label.changing.table$original.label == i))

    # Relabel the summary table
    cluster.label.summary$cluster <- index


    # Change to the new labeling system
    q_star_1_J_output <- lapply(1:length(q_star_1_J_output),
                                function(t) q_star_1_J_output[[t]][label.changing.table$original.label,])

    gamma_star_1_J_output <- lapply(1:length(gamma_star_1_J_output),
                                    function(t) gamma_star_1_J_output[[t]][label.changing.table$original.label])

    proj_prob_mean <- proj_prob_mean[label.changing.table$original.label,]
    proj_prob_lower <- proj_prob_lower[label.changing.table$original.label,]
    proj_prob_upper <- proj_prob_upper[label.changing.table$original.label,]

    gamma_mean <- gamma_mean[label.changing.table$original.label]
    ...
    Z <- lapply(1:M,
                function(m){
                  ...
                  sapply(1:C[m], function(c) which(label.changing.table$original.label == Z[[m]][c]))
                })
  ...
  if(isTRUE(relabel)){
    ...
    if(!is.null(project.to.front) & (length(project.to.front) != 0)){
      ...
      unique.class <- estimated.projection.df %>%
        filter(cluster %in% paste('cluster', (1:J)[label.changing.table$original.label %in% project.to.front])) %>%
        select(class) %>%
        pull() %>%
        unique()%>%
        sort()
    ...
    if(!is.null(project.to.middle) & (length(project.to.middle) != 0)){
      ...
      unique.class <- estimated.projection.df %>%
        filter(cluster %in% paste('cluster', (1:J)[label.changing.table$original.label %in% project.to.middle])) %>%
        select(class) %>%
        pull() %>%
        unique()%>%
        sort()
    ...
    if(!is.null(project.to.front.middle) & (length(project.to.front.middle) != 0)){
      ...
      unique.class <- estimated.projection.df %>%
        filter(cluster %in% paste('cluster', (1:J)[label.changing.table$original.label %in% project.to.front.middle])) %>%
        select(class) %>%
        pull() %>%
        unique()%>%
        sort()
    ...
    if(!is.null(project.to.back.middle) & (length(project.to.back.middle) != 0)){
      ...
      unique.class <- estimated.projection.df %>%
        filter(cluster %in% paste('cluster', (1:J)[label.changing.table$original.label %in% project.to.back.middle])) %>%
        select(class) %>%
        pull() %>%
        unique()%>%
        sort()
    ...
    if(!is.null(project.to.front.back) & (length(project.to.front.back) != 0)){
      ...
      unique.class <- estimated.projection.df %>%
        filter(cluster %in% paste('cluster', (1:J)[label.changing.table$original.label %in% project.to.front.back])) %>%
        select(class) %>%
        pull() %>%
        unique()%>%
        sort()
    ...
    if(!is.null(project.to.all.area) & (length(project.to.all.area) != 0)){
      ...
      unique.class <- estimated.projection.df %>%
        filter(cluster %in% paste('cluster', (1:J)[label.changing.table$original.label %in% project.to.all.area])) %>%
        select(class) %>%
        pull() %>%
        unique()%>%
        sort()
  ...
  if(relabel == TRUE){
    ...
    q_tilde_001 <- q_tilde_001[label.changing.table$original.label,]
  }
