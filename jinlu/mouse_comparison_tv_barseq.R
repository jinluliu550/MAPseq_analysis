...
mytv_dist = function(x,ind){
  xdim = dim(mcmc_all_barseq$omega_J_M_output[[1]])
  y = matrix(x[,ind],xdim[1],xdim[2])
  return(0.5*colSums(abs(x-y)))
}

m =  1 # index of mouse
tv_dist = lapply(mcmc_all_barseq$omega_J_M_output,mytv_dist, ind = m )
...
for (m in c(1:3)){
  tv_dist_m = lapply(mcmc_all_barseq$omega_J_M_output,mytv_dist, ind = m )
...
tv_lower = matrix(0, mcmc_all_barseq$M,mcmc_all_barseq$M)
tv_upper = matrix(0, mcmc_all_barseq$M,mcmc_all_barseq$M)
for (m in c(1:mcmc_all_barseq$M)){
  tv_dist_m = lapply(mcmc_all_barseq$omega_J_M_output,mytv_dist, ind = m )
...
tv_lower = data.frame(tv_lower, row.names = as.factor(c(1:mcmc_all_barseq$M)))
names(tv_lower) = as.factor(c(1:mcmc_all_barseq$M))
print(tv_lower)

tv_lower = data.frame(tv_lower, "Mouse 1" = as.factor(c(1:mcmc_all_barseq$M)))
names(tv_lower)[1:mcmc_all_barseq$M] = as.factor(c(1:mcmc_all_barseq$M))
...
tv_upper = data.frame(tv_upper, row.names = as.factor(c(1:mcmc_all_barseq$M)))
names(tv_upper) = as.factor(c(1:mcmc_all_barseq$M))
print(tv_upper)

tv_upper = data.frame(tv_upper, "Mouse 1" = as.factor(c(1:mcmc_all_barseq$M)))
names(tv_upper)[1:mcmc_all_barseq$M] = as.factor(c(1:mcmc_all_barseq$M))
