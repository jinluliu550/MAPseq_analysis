...
#Mouse 1
pp_m1 = projection_prob(1,100,mcmc_all_barseq)
ps_m1 = post_epstrength(1,100,mcmc_all_barseq)
cps_m1 = post_cpstrength(1,100,mcmc_all_barseq)

#Mouse 2
pp_m2 = projection_prob(2,100,mcmc_all_barseq)
ps_m2 = post_epstrength(2,100,mcmc_all_barseq)
cps_m2 = post_cpstrength(2,100,mcmc_all_barseq)

#Mouse 3
pp_m3 = projection_prob(3,100,mcmc_all_barseq)
ps_m3 = post_epstrength(3,100,mcmc_all_barseq)
cps_m3 = post_cpstrength(3,100,mcmc_all_barseq)
...
#Mouse 1
pp1 = data.frame(pp_m1$pprob_pair, row.names = rownames(data_barseq[[1]]))
names(pp1) = rownames(data_barseq[[1]])
print(pp1)

pp1 = data.frame(pp1, "Region B" = factor(rownames(data_barseq[[1]]), levels = rownames(data_barseq[[1]])))
names(pp1)[1:R] = rownames(data_barseq[[1]])
pp1 <-  pivot_longer(pp1,
                     cols = !Region.B,
                     names_to = "Region.A", 
                     values_to = "CP"
)
pp1$Region.A = factor(pp1$Region.A, levels = rownames(data_barseq[[1]]))
...
#Mouse 2
pp1 = data.frame(pp_m2$pprob_pair, row.names = rownames(data_barseq[[1]]))
names(pp1) = rownames(data_barseq[[1]])
print(pp1)
...
#Mouse 3
pp1 = data.frame(pp_m3$pprob_pair, row.names = rownames(data_barseq[[1]]))
names(pp1) = rownames(data_barseq[[1]])
print(pp1)
...
df$Region = factor(rep(rownames(data_barseq[[1]]), M), levels = rownames(data_barseq[[1]]))
